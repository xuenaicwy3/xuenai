...
def main(args):
  ...
  #  if os.path.exists("./best_weights") is False:
    ...
    # 实例化验证数据集
    test_dataset = MyDataSet(images_path=ceshi_images_path,
                            images_class=ceshi_images_label,
    ...
    tb_writer.add_scalar(tags[0], mean_loss)
    ...
    tags = ["loss", "accuracy", "learning_rate"]
    tb_writer.add_scalar(tags[0], mean_loss)
    tb_writer.add_scalar(tags[1], acc)
    tb_writer.add_scalar(tags[2], optimizer.param_groups[0]["lr"])

    # 打印最佳acc、f1
    print('Best_ACC: %.4f\t' % (
        round(acc, 3)))  # Best_ACC:SE_DENSE_TRASITION:0.9880
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='SEdensenet121_transition八分类.pth',
