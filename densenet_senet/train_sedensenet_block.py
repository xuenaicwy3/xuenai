...
from models1.se_densenet_block import densenet121, load_state_dict
...
def main(args):
  ...
  #  if os.path.exists("./best_weights") is False:
    ...
    train_accurate = []
    val_accurate = []
    save_path = './SEdensenet121_block八分类.pth'
    print(model.buffers)
    ...
    plt.legend()
    # 保存损失图
    plt.savefig('./loss_curve_SEDensenet_block八分类.png')
    # 显示损失图
    plt.show()
    ...
    plt.legend()
    # 保存损失图
    plt.savefig('./train_val_accurate_SEDensenet_block八分类.png')
    # 显示损失图
    plt.show()
