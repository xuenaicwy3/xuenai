...
def main(args):
  ...
  #  if os.path.exists("./best_weights") is False:
    ...
    train_accurate = []
    val_accurate = []
    save_path = './newdensenet201恶性四分类.pth'
    print(model.buffers)
    ...
    # 打印最佳acc、f1
    print('Best_ACC: %.4f\t' % (
        best_acc))  # 二分类Best_ACC: 0.9990  四分类Best_ACC: 0.9900
    ...
    plt.legend()
    # 保存损失图
    plt.savefig('./loss_curve_Densenet四分类.png')
    # 显示损失图
    plt.show()
    ...
    plt.legend()
    # 保存损失图
    plt.savefig('./train_val_accurate_Densenet四分类.png')
    # 显示损失图
    plt.show()
